...
function save_text($mysqli){
	try{
		$data = array();
		$text = $mysqli->real_escape_string(isset( $_POST['userText']['text'] ) ? $_POST['userText']['text'] : '');
		$id = $mysqli->real_escape_string( isset( $_POST['userText']['id'] ) ? $_POST['userText']['id'] : '');
		...
		if(empty($id)){
			$query = "INSERT INTO invoice (`id`, `text`) VALUES (NULL, '$text')";
		}else{
			$query = "UPDATE invoice SET `text` = '$text'";
		}
	
		if( $mysqli->query( $query ) ){
			$data['success'] = true;
			if(!empty($id))$data['message'] = 'Text updated successfully.';
			else $data['message'] = 'Text inserted successfully.';
			if(empty($id))$data['id'] = (int) $mysqli->insert_id;
			else $data['id'] = (int) $id;
		}else{
			throw new Exception( $mysqli->sqlstate.' - '. $mysqli->error );
